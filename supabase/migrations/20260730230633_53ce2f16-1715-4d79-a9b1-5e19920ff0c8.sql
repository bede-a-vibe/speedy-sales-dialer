...
DECLARE
  ...
  _in_golden_window boolean := _current_hour IN (10, 13);
...
BEGIN
  ...
  WITH visible_contacts AS (
    SELECT c.id, c.call_attempt_count, c.created_at, l.session_id AS lock_session_id,
      ...
      (_in_golden_window AND c.dm_phone IS NOT NULL AND c.dm_phone <> '') AS golden_dm_boost,
      (c.lead_type = 'inbound' AND c.created_at > now() - interval '24 hours' AND COALESCE(c.call_attempt_count, 0) = 0) AS fresh_inbound_boost,
      (
    ...
    WHERE c.status = 'uncalled' AND c.is_archived IS NOT TRUE
      ...
      AND (
        (c.next_followup_date IS NOT NULL AND c.next_followup_date <= now())
        OR c.lifecycle_stage IS NULL
        OR c.lifecycle_stage NOT IN ('won','lost','booked')
      )
  ...
  claimable_contacts AS (
    ...
    ORDER BY vc.fresh_inbound_boost DESC,
      CASE WHEN vc.fresh_inbound_boost THEN vc.created_at END DESC NULLS LAST,
      vc.golden_dm_boost DESC, vc.is_mobile_reachable DESC, vc.priority_score DESC, vc.call_attempt_count ASC, random()
  ...
  claimed_contacts AS (
    ...
    ORDER BY
      (c.lead_type = 'inbound' AND c.created_at > now() - interval '24 hours' AND COALESCE(c.call_attempt_count, 0) = 0) DESC,
      CASE WHEN (c.lead_type = 'inbound' AND c.created_at > now() - interval '24 hours' AND COALESCE(c.call_attempt_count, 0) = 0) THEN c.created_at END DESC NULLS LAST,
      (_in_golden_window AND c.dm_phone IS NOT NULL AND c.dm_phone <> '') DESC,
...
REVOKE EXECUTE ON FUNCTION public.claim_dialer_leads(uuid, integer, integer, text[], text[], text[], text, text, text, numeric, integer, text, text, text, text, boolean, text, boolean, boolean, text[], text[], text, text[], text, text, text, text, text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.claim_dialer_leads(uuid, integer, integer, text[], text[], text[], text, text, text, numeric, integer, text, text, text, text, boolean, text, boolean, boolean, text[], text[], text, text[], text, text, text, text, text) TO authenticated;