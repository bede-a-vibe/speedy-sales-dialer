...
-- Drop the current newest overloads only (leave older overloads untouched)
DROP FUNCTION IF EXISTS public.claim_dialer_leads(
  uuid, integer, integer, text[], text[], text[], text, text, text, numeric, integer,
  text, text, text, text, boolean, text, boolean, boolean, text[], text[], text, text[],
  text, text
);
DROP FUNCTION IF EXISTS public.get_dialer_queue_count(
  uuid, text[], text[], text[], text, text, text, numeric, integer,
  text, text, text, text, boolean, text, boolean, boolean, text[], text[], text, text[],
  text, text
);

-- ============================================================
-- claim_dialer_leads (newest overload) with _lead_source + _call_recency
-- and reachability-first priority score.
-- ============================================================
...
-- ============================================================
-- get_dialer_queue_count (newest overload) with _lead_source + _call_recency
-- ============================================================
...
-- ============================================================
-- get_dialer_filter_options: add `sources`
-- ============================================================
  ...
  SELECT jsonb_build_object(
    'industries', COALESCE((
      ...
      FROM (
        ...
        WHERE status = 'uncalled' AND is_dnc IS NOT TRUE AND disqualified IS NOT TRUE
    ...
    'channels', COALESCE((
      ...
      FROM (
        ...
        FROM public.contacts
        WHERE status = 'uncalled' AND is_dnc IS NOT TRUE AND disqualified IS NOT TRUE
          AND lead_channel IS NOT NULL AND lead_channel <> ''
    ...
    'sources', COALESCE((
      SELECT jsonb_agg(jsonb_build_object('value', value, 'count', c) ORDER BY c DESC)
      FROM (
        SELECT lead_source AS value, COUNT(*)::int AS c
        FROM public.contacts
        WHERE status = 'uncalled' AND is_dnc IS NOT TRUE AND disqualified IS NOT TRUE
