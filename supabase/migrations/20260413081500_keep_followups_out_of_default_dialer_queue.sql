-- Keep scheduled follow-ups out of the default prospecting dialer queue.
--
-- Follow-ups are warmer leads and should live in their own workflow instead of
-- mixing back into the generic uncalled/prospecting queue once their due time arrives.
-- Booked contacts were already excluded; this change makes the default queue
-- prospecting-only unless a future explicit follow-up dialer path is added.
...
BEGIN
  ...
  WITH visible_contacts AS (
    ...
    WHERE c.status = 'uncalled'
      ...
      AND c.next_followup_date IS NULL
      ...
      AND (
        c.phone_type NOT IN ('landline', 'business_line')
        OR (c.dm_phone IS NOT NULL AND c.dm_phone <> '')
        OR _phone_type IN ('landline', 'business_line')
      )
      AND (
