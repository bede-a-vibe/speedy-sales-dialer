...
-- Extend claim_dialer_leads (full overload) with _lead_type + _lead_channel trailing params.
CREATE OR REPLACE FUNCTION public.claim_dialer_leads(
  ...
  _dnc_reasons text[] DEFAULT NULL::text[],
  _dq_reasons text[] DEFAULT NULL::text[],
  _has_existing_agency text DEFAULT NULL::text,
  _existing_agency_services text[] DEFAULT NULL::text[],
  _lead_type text DEFAULT NULL::text,
  _lead_channel text DEFAULT NULL::text
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
-- Extend get_dialer_queue_count with _lead_type + _lead_channel trailing params.
