-- Phase 5: soft-archive + merged_into columns, dedup RPCs, and archived filter in queue functions.

ALTER TABLE public.contacts 
  ADD COLUMN IF NOT EXISTS is_archived boolean NOT NULL DEFAULT false,
  ADD COLUMN IF NOT EXISTS merged_into_contact_id uuid REFERENCES public.contacts(id) ON DELETE SET NULL;

CREATE INDEX IF NOT EXISTS idx_contacts_active ON public.contacts(id) WHERE is_archived = false;

CREATE OR REPLACE FUNCTION public.claim_dialer_leads(_session_id uuid, _claim_size integer DEFAULT 25, _lock_minutes integer DEFAULT 15, _industries text[] DEFAULT NULL::text[], _states text[] DEFAULT NULL::text[], _trade_types text[] DEFAULT NULL::text[], _work_type text DEFAULT NULL::text, _business_size text DEFAULT NULL::text, _prospect_tier text DEFAULT NULL::text, _min_gbp_rating numeric DEFAULT NULL::numeric, _min_review_count integer DEFAULT NULL::integer, _has_google_ads text DEFAULT NULL::text, _has_facebook_ads text DEFAULT NULL::text, _buying_signal_strength text DEFAULT NULL::text, _phone_type text DEFAULT NULL::text, _has_dm_phone boolean DEFAULT NULL::boolean, _contact_owner text DEFAULT NULL::text, _include_dnc boolean DEFAULT false, _include_disqualified boolean DEFAULT false, _dnc_reasons text[] DEFAULT NULL::text[], _dq_reasons text[] DEFAULT NULL::text[], _has_existing_agency text DEFAULT NULL::text, _existing_agency_services text[] DEFAULT NULL::text[], _lead_type text DEFAULT NULL::text, _lead_channel text DEFAULT NULL::text, _lead_source text DEFAULT NULL::text, _call_recency text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
CREATE OR REPLACE FUNCTION public.get_dialer_queue_count(_session_id uuid, _industries text[] DEFAULT NULL::text[], _states text[] DEFAULT NULL::text[], _trade_types text[] DEFAULT NULL::text[], _work_type text DEFAULT NULL::text, _business_size text DEFAULT NULL::text, _prospect_tier text DEFAULT NULL::text, _min_gbp_rating numeric DEFAULT NULL::numeric, _min_review_count integer DEFAULT NULL::integer, _has_google_ads text DEFAULT NULL::text, _has_facebook_ads text DEFAULT NULL::text, _buying_signal_strength text DEFAULT NULL::text, _phone_type text DEFAULT NULL::text, _has_dm_phone boolean DEFAULT NULL::boolean, _contact_owner text DEFAULT NULL::text)
 RETURNS integer
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.get_dialer_queue_count(_session_id uuid, _industries text[] DEFAULT NULL::text[], _states text[] DEFAULT NULL::text[], _trade_types text[] DEFAULT NULL::text[], _work_type text DEFAULT NULL::text, _business_size text DEFAULT NULL::text, _prospect_tier text DEFAULT NULL::text, _min_gbp_rating numeric DEFAULT NULL::numeric, _min_review_count integer DEFAULT NULL::integer, _has_google_ads text DEFAULT NULL::text, _has_facebook_ads text DEFAULT NULL::text, _buying_signal_strength text DEFAULT NULL::text, _phone_type text DEFAULT NULL::text, _has_dm_phone boolean DEFAULT NULL::boolean, _contact_owner text DEFAULT NULL::text, _include_dnc boolean DEFAULT false, _include_disqualified boolean DEFAULT false, _dnc_reasons text[] DEFAULT NULL::text[], _dq_reasons text[] DEFAULT NULL::text[], _has_existing_agency text DEFAULT NULL::text, _existing_agency_services text[] DEFAULT NULL::text[])
 RETURNS integer
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.merge_contacts(_master uuid, _losers uuid[])
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
DECLARE
  _result jsonb;
BEGIN
  IF _master IS NULL OR _losers IS NULL OR array_length(_losers, 1) IS NULL THEN
    RAISE EXCEPTION 'merge_contacts requires master and at least one loser';
  END IF;
  IF _master = ANY(_losers) THEN
    RAISE EXCEPTION 'merge_contacts: master cannot appear in losers array';
  END IF;

  UPDATE public.contacts m SET
    contact_person = COALESCE(m.contact_person, l.contact_person),
    email = COALESCE(m.email, l.email),
    website = COALESCE(m.website, l.website),
    gmb_link = COALESCE(m.gmb_link, l.gmb_link),
    city = COALESCE(m.city, l.city),
    state = COALESCE(m.state, l.state),
    industry = COALESCE(m.industry, l.industry),
    dm_name = COALESCE(m.dm_name, l.dm_name),
    dm_phone = COALESCE(m.dm_phone, l.dm_phone),
    dm_email = COALESCE(m.dm_email, l.dm_email),
    dm_role = COALESCE(m.dm_role, l.dm_role),
    gatekeeper_name = COALESCE(m.gatekeeper_name, l.gatekeeper_name),
    ghl_contact_id = COALESCE(m.ghl_contact_id, l.ghl_contact_id),
    owner_id = COALESCE(m.owner_id, l.owner_id),
    lifecycle_stage = COALESCE(m.lifecycle_stage, l.lifecycle_stage),
    best_time_to_call = COALESCE(m.best_time_to_call, l.best_time_to_call),
    best_route_to_decision_maker = COALESCE(m.best_route_to_decision_maker, l.best_route_to_decision_maker),
    updated_at = now()
  FROM public.contacts l
  WHERE m.id = _master AND l.id = ANY(_losers);

  UPDATE public.call_logs SET contact_id = _master WHERE contact_id = ANY(_losers);
  UPDATE public.contact_notes SET contact_id = _master WHERE contact_id = ANY(_losers);
  UPDATE public.pipeline_items SET contact_id = _master WHERE contact_id = ANY(_losers);
  UPDATE public.dialpad_calls SET contact_id = _master WHERE contact_id = ANY(_losers);

  UPDATE public.contacts
     SET is_archived = true,
         merged_into_contact_id = _master,
         updated_at = now()
   WHERE id = ANY(_losers);

  _result := jsonb_build_object(
    'master_id', _master,
    'archived_ids', to_jsonb(_losers),
    'archived_count', array_length(_losers, 1)
  );
  RETURN _result;
END;
$fn$;
...
CREATE OR REPLACE FUNCTION public.find_exact_phone_duplicate_groups()
RETURNS TABLE(normalized_phone text, contact_ids uuid[], contact_count integer)
LANGUAGE sql
STABLE
SECURITY DEFINER
SET search_path = public
AS $fn$
  SELECT regexp_replace(phone,'[^0-9]','','g') AS normalized_phone,
         array_agg(id ORDER BY updated_at DESC NULLS LAST) AS contact_ids,
...
GRANT EXECUTE ON FUNCTION public.find_exact_phone_duplicate_groups() TO authenticated;
