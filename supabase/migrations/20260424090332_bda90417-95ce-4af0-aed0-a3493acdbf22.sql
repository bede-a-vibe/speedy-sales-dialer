-- Re-create claim_dialer_leads with case-insensitive ad-status matching
-- and a trade_type UI→DB normalization mapping. Same signature as before.
...
BEGIN
  ...
  -- Normalize trade_types: accept both UI labels and stored canonical values.
  -- For each input, also include the canonical equivalent so e.g. picking
  -- "Plumbers" also matches contacts with trade_type = 'Plumbing'.
...
-- Same fixes applied to get_dialer_queue_count
