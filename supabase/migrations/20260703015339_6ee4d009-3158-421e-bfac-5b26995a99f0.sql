...
-- 1. Extend call_outcome enum (safe: not referenced literally elsewhere in this tx)
ALTER TYPE public.call_outcome ADD VALUE IF NOT EXISTS 'disqualified';
...
ALTER TABLE public.contacts
  ...
  ADD COLUMN IF NOT EXISTS dnc_recorded_at timestamptz,
  ADD COLUMN IF NOT EXISTS has_existing_agency boolean,
  ADD COLUMN IF NOT EXISTS existing_agency_name text,
  ADD COLUMN IF NOT EXISTS existing_agency_services text[] NOT NULL DEFAULT '{}',
  ADD COLUMN IF NOT EXISTS existing_agency_notes text;

-- 3. Validation trigger for reason values (immutable check constraints don't work on text easily; use trigger)
CREATE OR REPLACE FUNCTION public.validate_contact_disposition()
RETURNS trigger
LANGUAGE plpgsql
SET search_path = public
AS $$
BEGIN
  IF NEW.disqualified_reason IS NOT NULL
     AND NEW.disqualified_reason NOT IN ('financially_not_qualified', 'not_looking_to_grow') THEN
    RAISE EXCEPTION 'Invalid disqualified_reason: %', NEW.disqualified_reason;
  END IF;

  IF NEW.dnc_reason IS NOT NULL
     AND NEW.dnc_reason NOT IN ('requested_removal', 'abusive_or_hostile', 'wrong_number_repeat', 'other') THEN
    RAISE EXCEPTION 'Invalid dnc_reason: %', NEW.dnc_reason;
  END IF;

  -- Auto-stamp timestamps when flags flip on
  IF NEW.disqualified = true AND (TG_OP = 'INSERT' OR OLD.disqualified = false) AND NEW.disqualified_at IS NULL THEN
    NEW.disqualified_at := now();
  END IF;
  IF NEW.disqualified = false THEN
    NEW.disqualified_at := NULL;
    NEW.disqualified_reason := NULL;
    NEW.disqualified_notes := NULL;
  END IF;

  IF NEW.is_dnc = true AND (TG_OP = 'INSERT' OR OLD.is_dnc = false) AND NEW.dnc_recorded_at IS NULL THEN
    NEW.dnc_recorded_at := now();
  END IF;
  IF NEW.is_dnc = false THEN
    NEW.dnc_recorded_at := NULL;
    NEW.dnc_reason := NULL;
    NEW.dnc_notes := NULL;
  END IF;

  -- Validate agency services subset
  IF NEW.existing_agency_services IS NOT NULL AND array_length(NEW.existing_agency_services, 1) > 0 THEN
    IF EXISTS (
      SELECT 1
      FROM unnest(NEW.existing_agency_services) AS s
      WHERE s NOT IN ('seo', 'google_ads', 'meta_ads', 'website', 'other')
    ) THEN
      RAISE EXCEPTION 'Invalid existing_agency_services entry';
    END IF;
  END IF;
...
DROP TRIGGER IF EXISTS trg_validate_contact_disposition ON public.contacts;
CREATE TRIGGER trg_validate_contact_disposition
BEFORE INSERT OR UPDATE ON public.contacts
FOR EACH ROW EXECUTE FUNCTION public.validate_contact_disposition();

-- 4. Indexes
CREATE INDEX IF NOT EXISTS idx_contacts_disqualified ON public.contacts(disqualified);
CREATE INDEX IF NOT EXISTS idx_contacts_disqualified_reason ON public.contacts(disqualified_reason);
CREATE INDEX IF NOT EXISTS idx_contacts_has_existing_agency ON public.contacts(has_existing_agency);
CREATE INDEX IF NOT EXISTS idx_contacts_existing_agency_services ON public.contacts USING GIN (existing_agency_services);

-- 5. Update dialer RPCs to exclude disqualified + accept new filter parameters
...
BEGIN
  ...
  WITH visible_contacts AS (
    ...
    WHERE c.status = 'uncalled'
      ...
      AND (_industries IS NULL OR c.industry = ANY(_industries))
      AND (_states IS NULL OR UPPER(COALESCE(c.state, '')) = ANY(ARRAY(SELECT UPPER(value) FROM unnest(_states) AS value)))
      AND (_normalized_trade_types IS NULL
        OR c.trade_type = ANY(_normalized_trade_types)
        OR (c.trade_type IS NULL AND c.industry = ANY(_normalized_trade_types)))
      AND (_work_type IS NULL OR _work_type = 'all' OR c.work_type = _work_type)
      AND (_business_size IS NULL OR _business_size = 'all' OR c.business_size = _business_size)
