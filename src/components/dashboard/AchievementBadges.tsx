...
type AchievementTier = "daily" | "weekly" | "monthly" | "lifetime";

const TIER_CONFIG: Record<AchievementTier, { label: string; cols: string }> = {
  daily: { label: "Daily", cols: "grid-cols-4 lg:grid-cols-7" },
...
export function AchievementBadges() {
  ...
  const [tab, setTab] = useState<AchievementTier>("daily");
  ...
  const current = tiers[tab];
  const unlockedCount = current.filter((a) => a.unlocked).length;
  ...
  const confettiActive = useConfettiTrigger(totalUnlocked > 0);

  const tierUnlocked = (tier: AchievementTier) => tiers[tier].filter((a) => a.unlocked).length;
  ...
  return (
    <div className="relative rounded-xl border border-border bg-card p-5 overflow-hidden">
      ...
      <Tabs value={tab} onValueChange={(v) => setTab(v as AchievementTier)}>
        ...
        {(Object.keys(TIER_CONFIG) as AchievementTier[]).map((tier) => {
          ...
          return (
            <TabsContent key={tier} value={tier}>
              ...
              <div className={cn("grid gap-2", TIER_CONFIG[tier].cols)}>
                {achievements.map((a) => (
